...
| 项 | 值 |
|---|---|
| 基准成绩 | **v5 干净单层库 81.4%（407/500）**；v4 三语混合库 82.2%（410/499）；v3 轮 74.2%；基线 28.2% |
...
- v5 干净库 500 题全量评分（tag=v5-500）已完成：**407/500 = 81.4%**。它是第一个可干净归因的单遍抽取基线；v4 的 82.2% 来自三语混合库，不能当作单变量效果。
- 分型变化（v4 → v5）：multi-session +4，abstention +3，single-session-user +1；knowledge-update 逐题同分；temporal −2，single-session-preference −4，single-session-assistant −5。干净化带来 +8，失去三次独立抽取覆盖带来 −11，净效应 −0.8pp。
- 结果确认两条判断：干净化确实改善聚合与拒答；跨 key 病灶对干净/混合库无反应，KU 仍为 65/72，需要结构修复而不是重复重摄。
