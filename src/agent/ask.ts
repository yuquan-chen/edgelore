...
export function buildAskPrompt(question: string, memories: readonly string[], now?: string): string {
  ...
  return [
    ...
    "5. Combine multiple entries when the answer spans several memories. When asked for",
    "   a total across named activities/items, add their recorded durations, costs, or",
    "   counts; do not collapse overlap unless the question asks for elapsed calendar time.",
