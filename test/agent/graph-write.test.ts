...
test("graph write: structural relations are governed hyperedge nodes", () => {
  forBackend((graph) => {
    const result = commitGraphWritePlan(
      graph,
      {
        entities: [
          ...
          { ref: "decoration", type: "concept:category", key: "decoration", scope: "global" },
        ],
        facts: [
          {
            ref: "fact",
        ...
        relations: [{ type: "core:about", from: "fact", to: "interior" }],
        relationAssertions: [
          {
            ref: "partOf",
          ...
          },
          {
            ...
            bindings: { dimension: "dimension", subject: "interior" },
            supportedBy: ["fact"],
          },
          {
            ref: "relationContext",
            predicate: "ontology:coexists_with",
            bindings: { structure: "partOf", classification: "classifiedAs" },
            supportedBy: ["fact"],
          },
        ],
      },
    ...
    assert.equal(result.refs.dimension, result.captures[0]?.dimensionId);
    assert.equal(result.createdRelationIds.length, 4);
    assert.equal(graph.queryNodes({ type: "core:relation" }).length, 4);
    assert.equal(graph.queryEdges({ type: "core:supports" }).length, 4);
    const relations = graph.queryNodes({ type: "core:relation" }) as Array<{
      id: string;
      predicate: string;
      bindings: Record<string, string>;
      state: string;
    }>;
    assert.ok(relations.every((relation) => relation.state === "accepted"));
    const nested = relations.find((relation) => relation.predicate === "ontology:coexists_with");
    assert.equal(nested?.bindings.structure, result.refs.partOf);
    assert.equal(nested?.bindings.classification, result.refs.classifiedAs);
  });
});

test("graph write: repeated structural relation reuses identity and accumulates support", () => {
  forBackend((graph) => {
    const plan = (value: string, saidBy: "assistant" | "user"): GraphWritePlan => ({
      entities: [
        { ref: "car", type: "vehicle:car", key: "current-car" },
        { ref: "interior", type: "vehicle:interior", key: "current-car-interior" },
      ],
      facts: [
        {
          ref: "fact",
          content: { dimensionKey: "carInterior", value, saidBy },
        },
      ],
      relations: [],
      relationAssertions: [
    ...
    const first = commitGraphWritePlan(
      graph,
      plan("The car interior is part of the car", "assistant"),
      aliceCtx,
    );
    assert.equal(graph.getNode(first.refs.partOf)?.state, "tentative");

    const second = commitGraphWritePlan(
      graph,
      plan("Seat covers were installed in the car interior", "user"),
      { ...aliceCtx, source_refs: ["session:interior-followup"] },
    );

    assert.equal(second.refs.partOf, first.refs.partOf);
    assert.equal(graph.getNode(first.refs.partOf)?.state, "accepted");
    assert.equal(graph.queryNodes({ type: "core:relation" }).length, 1);
    assert.equal(graph.queryEdges({ type: "core:supports", to: first.refs.partOf }).length, 2);
  });
});
